...
public class DialogCommons
{
	 ...
	 *        buttons
	 */
	protected static void setupGeneralDialog(MessageBox box, boolean resizable, boolean closeWithAClickOnAnyButton)
	{
		box.getWindow().setWindowMode(WindowMode.NORMAL);
		box.getWindow().setResizable(resizable);
		box.getWindow().setDraggable(false);
		box.getWindow().setSizeUndefined();
		box.setAutoClose(closeWithAClickOnAnyButton);
		box.getWindow().focus(); // UI.getCurrent().setFocusedComponent(box.getWindow());
	}
	 ...
	 * </ul>
	 */
	protected static void bindActionsToKeyboard(MessageBox box, Button enterButton, boolean escapeKeyClosesDialog)
	{
		if (enterButton != null)
		{
			enterButton.setClickShortcut(KeyCodes.KEY_ENTER, null);
		}
		if (escapeKeyClosesDialog)
		{
