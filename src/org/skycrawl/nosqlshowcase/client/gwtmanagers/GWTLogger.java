...
/**
 * A utility class that forwards received logging messages to the server.
 * 
 * @author SkyCrawl
 */
public class GWTLogger
{
	...
	/**
	 * @param serverRPC
	 *        the server UI to send logging messages to
	 */
	...
	/**
	 * Sends a message and a throwable to the server where they will be logged
	 * as an error. Only use this
	 * with your own Java exceptions in GWT code. Native exceptions are useless
	 * for a GWT developer and {@link #logUncaughtNativeClientException} should
	 * be used instead.
	 */
	...
	/**
	 * Sends a notification to the server that a native and unpredicted
	 * exception occured in the client code.
	 */
	...
	private static boolean checkLogger()
	{
		if (serverRPC == null)
		{
			throw new NullPointerException(
					"Remote client logger was needed but had not been set. Ensure that your main UI is extended"
